...
class ProductLabelPricelist(models.TransientModel):
    ...
    def print_report(self):
        ...
        res = self.with_context(ctx).env['product.product'].browse(self._context.get('active_ids', []))
        p_ids=[]
        h = self.altura
        if self.discount:
            rows = 4
        ...
        return self.env.ref('product_custom_label_anz.action_print_label_docs_pricelist').report_action(ppl.browse(p_ids))
