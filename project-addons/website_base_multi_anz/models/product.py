...
class ProductPublicCategory(models.Model):
    ...
    public_categ_tag_ids = fields.Many2many(
        "product.public.category.tag",
...
class ProductTemplate(models.Model):
    ...
    inventory_availability = fields.Selection(
        selection_add=[
            (
                "always_virtual",
                _(
                    "Show future and current inventory on website and prevent sales if not enough stock"
                ),
            ),
            (
                "threshold_virtual",
                _(
                    "Show future and current inventory below a threshold and prevent sales if not enough stock"
                ),
            ),
        ]
    )
    ...
    def act_tsw(self, website, stock_website_published=True):
        if stock_website_published:
            ## En teoría es si pero ...
            ctx = self._context.copy()
            swp_fields = ["always_virtual", "threshold_virtual"]
            inventory_availability = (
                self.inventory_availability or website.inventory_availability
            )
    ...
    def get_web_max_qty(self):
        ...
        if (
            self.inventory_availability in ["threshold", "threshold_virtual"]
            and self.available_threshold > 0
        ):
            max_qty = max(0, max_qty - self.available_threshold)
        return max_qty
    ...
    def act_stock_published(self):
        ...
        template_stock_web_error_domain = [('product_id', 'in', template_ids.ids), ("website_id", "=", False)]
        self.env["template.stock.web"].search(template_stock_web_error_domain).unlink()
        all_websites = self.env["website"].search([('name', '!=', False)])
        for template in template_ids:
            cont += 1
            _logger.info(
                "{}: {} de {}: {}".format(
                    "Update Website Visibility by Stock", cont, tot, template.name
                )
            )
            template_websites = template.website_ids
            for website in all_websites:
                if not template_websites or website in template_websites:
                    template.act_tsw(website)
                else:
                    template.act_tsw(website, False)
