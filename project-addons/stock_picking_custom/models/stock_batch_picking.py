...
class StockBatchPicking(models.Model):
    ...
    @api.multi
    def batch_force_set_qty_done(self):
        ctx = self._context.copy()
        for batch in self:
            batch.picking_ids.with_context(ctx).picking_force_set_qty_done()

    @api.multi
    def batch_force_set_assigned_qty_done(self):
        ctx = self._context.copy()
        for batch in self:
            batch.picking_ids.with_context(ctx).picking_force_set_assigned_qty_done()

    @api.multi
    def batch_force_reset_qties(self):
        ctx = self._context.copy()
        for batch in self:
            batch.picking_ids.with_context(ctx).force_reset_qties()