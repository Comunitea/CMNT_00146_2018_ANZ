...
class SaleOrder(models.Model):
    ...
    def action_view_order_lines_template_group(self):
        ...
        tree_view = model_data.get_object_reference(
            'sale_custom_anz', 'view_sale_order_line_group_template_tree')

        action = self.env.ref(
            'sale_custom_anz.view_sale_order_line_group_template_tree_action').read()[0]
        action['views'] = {
            (tree_view and tree_view[1] or False, 'tree'),}
        action['domain'] = [('order_id', 'in', self.order_id)]
...
class SaleOrderLineTemplateGroup(models.Model):
    ...
    _auto = False
    _rec_name = 'template'
    _order = 'sequence, template'

    @api.multi
    @api.depends('product_tmpl_id')
    def get_line_name(self):
        for line in self:
            line.line_name = line.product_tmpl_id.name + line.suffix_variant + "{}".format(line.discount)
